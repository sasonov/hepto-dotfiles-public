...
Log file: `local filesskill-performance.jsonl`
...
3. **Append to JSONL**: Write one line to `local filesskill-performance.jsonl`
...
cat local filesskill-performance.jsonl | jq -r '.skill' | sort | uniq -c | sort -rn

# Success rate per skill
cat local filesskill-performance.jsonl | jq -r '[.skill, .outcome] | join(" ")' | sort | uniq -c

# Average iterations per skill (lower is better)
cat local filesskill-performance.jsonl | jq -r '[.skill, .iterations] | join(" ")' | awk '{sum[$1]+=$2; count[$1]++} END {for (k in sum) printf "%s %.1f\n", k, sum[k]/count[k]}'

# Recent failures (last 7 days)
cat local filesskill-performance.jsonl | jq -r 'select(.outcome != "success") | [.timestamp, .skill, .task] | join(" | ")'
```
...
Store weekly summaries in `local filesweekly-skill-reviews/` as markdown files:
...
1. Reads `local filesskill-performance.jsonl`
...
4. Logs the evolution in `local filesskill-evolution-log.jsonl`
